package com.jiqian.display 
{
	...
	public class ScaleBitmap extends Bitmap {
		...
		public function ScaleBitmap(bmpData : BitmapData = null, _rect:Rectangle = null, pixelSnapping : String = "auto", smoothing : Boolean = false) {
			...
			if(!_rect)
			{
				_rect = new Rectangle(int(bmpData.width / 2), int(bmpData.height / 2), 1, 1);
				if (!(bmpData.width % 2))
				{
					_rect.x = (bmpData.width - 2) / 2;
					_rect.width = 2;
				}
				if (!(bmpData.height % 2))
				{
					_rect.y = (bmpData.height - 2) / 2;
					_rect.height = 2;
				}
			}
			this.scale9Grid = _rect;
		}
